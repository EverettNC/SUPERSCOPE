import{a as e}from"./store-6y1Astwc.js";function t(e){return`scope-probe-${e}.html`}function n(e,n=t(e)){let r=i(e),a=new Blob([r],{type:`text/html;charset=utf-8`}),o=URL.createObjectURL(a),s=document.createElement(`a`);s.href=o,s.download=n,s.rel=`noopener`,document.body.appendChild(s),s.click(),s.remove(),window.setTimeout(()=>URL.revokeObjectURL(o),2e3)}async function r(r,a=t(r)){let o=i(r),s=window.showSaveFilePicker;if(typeof s==`function`)try{let e=await(await s({suggestedName:a,types:[{description:`HTML file`,accept:{"text/html":[`.html`]}}]})).createWritable();return await e.write(o),await e.close(),{html:o,saved:!0,copied:!1}}catch(e){if(e instanceof DOMException&&e.name===`AbortError`)return{html:o,saved:!1,copied:!1}}return n(r,a),{html:o,saved:!1,copied:await e(o)}}function i(e){let t=JSON.stringify(e);return`<!DOCTYPE html>
<html lang="en">
<head>
<meta charset="utf-8"/>
...
function pickMaleVoice() {
  ...
  const prefer = ["Microsoft Guy","Google US English","Microsoft David","Microsoft Mark","Alex","Daniel","Fred"];
  for (let i = 0; i < prefer.length; i++) {
    const hit = voices.find((v) => v.name.toLowerCase().indexOf(prefer[i].toLowerCase()) !== -1);
    if (hit) return hit;
  }
  const skip = /ava|aria|jenny|zira|samantha|female|linda|sonia|emma|michelle|susan|hazel|moira|karen|heera|tessa|siri|fiona/i;
  return voices.find((v) => /guy|david|mark|andrew|james|ryan|tony|davis|christopher|eric|george|tom|daniel|alex|fred|male/i.test(v.name) && !skip.test(v.name))
    || voices.find((v) => v.lang && v.lang.toLowerCase().indexOf("en") === 0 && !skip.test(v.name))
    || null;
}
...
async function run() {
  ...
  $("speak").onclick = () => {
    if (window.speechSynthesis && window.speechSynthesis.speaking) {
      window.speechSynthesis.cancel();
      $("copied").textContent = "Stopped.";
      $("speak").textContent = "Read this aloud";
      return;
    }
    const ok = speakText(spoken);
    $("copied").textContent = ok ? "Reading now. Unmute this computer if you hear nothing." : "This browser cannot speak. Read the findings above.";
