...
export function probeFileName(code: string): string {
  return `scope-probe-${code}.html`;
}

export function downloadProbeFile(code: string, filename = probeFileName(code)) {
...
export async function saveProbeFile(
  code: string,
  filename = probeFileName(code),
): Promise<{ html: string; saved: boolean; copied: boolean }> {
  const html = buildProbeHtml(code);
  const picker = (
    window as Window & {
  ...
  if (typeof picker === "function") {
    try {
      ...
      return { html, saved: true, copied: false };
    } catch (err) {
      if (err instanceof DOMException && err.name === "AbortError") {
        return { html, saved: false, copied: false };
      }
    }
  }

  downloadProbeFile(code, filename);
  const copied = await copyText(html);
  return { html, saved: false, copied };
}

export function buildProbeHtml(code: string): string {
